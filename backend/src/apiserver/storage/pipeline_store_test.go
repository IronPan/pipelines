...
//	return &model.Pipeline{Name: name, Parameters: `[{"Name": "param1"}]`, Status: model.PipelineVersionReady}
//}
//
...
//	pipelineStore.CreatePipeline(&model.Pipeline{Name: "pipeline3", Status: model.PipelineVersionCreating})
//
...
//	}
//	err = pipelineStore.UpdatePipelineStatus(pipeline.UUID, model.PipelineVersionDeleting)
//	assert.Nil(t, err)
//	pipeline, err = pipelineStore.GetPipelineWithStatus(fakeUUID, model.PipelineVersionDeleting)
//	assert.Nil(t, err)
...
//	pipelineStore := NewPipelineStore(db, util.NewFakeTimeForEpoch(), util.NewFakeUUIDGeneratorOrFatal(fakeUUID, nil))
//	db.Close()
//	err := pipelineStore.UpdatePipelineStatus(fakeUUID, model.PipelineVersionDeleting)
//	assert.Equal(t, codes.Internal, err.(*util.UserError).ExternalStatusCode())
//}
